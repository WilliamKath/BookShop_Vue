<template>
    <el-table :data="recordsList" style="width: 100%">
        ...
        <el-table-column label="操作">
            <template slot-scope="scope">
                <span v-if="scope.row.goodsState != 0">
                    <el-tag type="info" effect="dark">
                        您已发货
                    </el-tag>
                </span>
                <span v-else>
                    <el-popconfirm title="确定已发货吗？" @confirm=changeState(scope.row)>
                        <el-button type="primary" slot="reference">我已发货</el-button>
                    </el-popconfirm>
                </span>
            </template>
            <!-- <el-button type="primary">我已发货</el-button> -->
        </el-table-column>
...
export default {
    data() {
        return {
            sellerId: '', //当前用户的ID
            recordsList: [], //查询到的记录列表
        }
    },
    created() {
        this.sellerId = this.cookie.getCookie("LoginId");
        this.getRecordsList();
    },
    mounted() {
    },
